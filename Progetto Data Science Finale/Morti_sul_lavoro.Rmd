...
genderYearlyMean = data.frame(gender, genderMeanDeathCount = as.integer(genderTot$genderDeathCount/7))
...
ageYearlyMean = data.frame(ageRange, ageMeanDeathCount = as.integer(ageTot$ageSum/7))
...
monthYearlyMean = data.frame(month, monthMeanDeathCount = as.integer(monthTot$monthDeathCount/7)) #stimo una media dei morti ogni anno in ogni periodo dell'anno

yearlyDeaths = (sum(monthYearlyMean$monthMeanDeathCount)) #calcolo quante morti sul posto di lavoro ci sono mediamente all'anno
...
Dati 6 anni da analizzare, è importante cominciare ad organizzare i dati per poterli usare al meglio.

```{r Importazione csv}
#importo le tabelle con i dati sulle date, età, sesso della vittima e 
#stato di appartenenza dei casi negli USA
library(readr)
library(tidyr)
library(dplyr)
...
C'è una distinzione di genere? si organizzano i dati in maniera appropriata per scoprirlo.
...
pie(genderYearlyMean$genderMeanDeathCount, main = "Morti sul posto di lavoro", 
...
Ci sono distinzioni di età? si organizzano i dati in maniera appropriata per scoprirlo.
...
barplot(ageYearlyMean$ageMeanDeathCount, xlab = "Age", ylab = "Count", 
        main = "Morti sul posto di lavoro", names.arg = ageTot$ageRange)
```

La fascia più colpita è quella dei 51-75 anni. Causa?

Non c'è una vera risposta, si pensa al tipo di lavoro svolto: internet ha cambiato ed evoluto il concetto di lavoro.
...
C'è distinzione nel periodo dell'anno? si organizzano i dati in maniera appropriata per scoprirlo.
...
```{r Grafico dei mesi}
barplot(monthYearlyMean$monthMeanDeathCount, xlab = "Month", ylab = "Count", 
        main = "Morti sul posto di lavoro", 
        names.arg = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug",
                      "Sep", "Oct", "Nov", "Dec"))
boxplot(monthYearlyMean$monthMeanDeathCount,  main = "Morti sul posto di lavoro")
summary(monthYearlyMean$monthMeanDeathCount)
```
